define(["widgets/brease/TableWidget/libs/Renderer"], function (SuperClass) {
  "use strict";
  /**
   * @class widgets.brXtended.UserList.libs.Renderer
  ...
  p.setFilter = function () {
    ...
    if (!this.internal.filterSet) {
      ...
      $.fn.dataTable.ext.search.push(function (settings, data, dataIndex, row) {
        var elem = $(settings.nTable).closest(
          '[data-brease-widget="widgets/brXtended/UserList"]'
        )[0];
        if (
          !elem ||
          elem.id !== self.widget.elem.id ||
          self.widget.model.getPreparedData().length === 0
        )
          return true;

        var accVal = self.widget.settings.config.filter.length === 0,
          accAnd = true;
        for (var i = 0; i < self.widget.settings.config.filter.length; i += 1) {
          var fil = self.widget.settings.config.filter[i];

          var vals = self._getComparativeAndOriginalValue(
              fil,
              self.widget.model.currentData[dataIndex]
            ),
            origVal = vals[1],
            compVal = vals[0];
  ...
  p._getComparativeAndOriginalValue = function (fil, row) {
    ...
    if (fil.data === "LastLogin") {
      compVal = this._fixTimestamp(fil.comp.split("Z")[0]).getTime();
      origVal = this._fixTimestamp(row.LastLogin.split("Z")[0]).getTime();
    } else {
      compVal = fil.comp;
  ...
  p._rowEligibility = function (currState, currSev, style) {
    ...
    if (currState === style.statePos || style.statePos === 5) {
      if (style.sevOneUse && style.sevTwoUse && style.sevTwoOp === 0) {
        return (
          this._getFilterStatement(currSev, style.sevOnePos, style.sevOne) &&
          this._getFilterStatement(currSev, style.sevTwoPos, style.sevTwo)
        );
  ...
  p._centerImages = function () {
    if (this.widget.el) {
      var margin =
        "calc(50% - " +
        parseInt(this.widget.el.find("td > img").width()).toString() +
        ")";
  ...
  p._colorMeBlue = function () {
    if (this.widget.model.currentData === undefined) {
      return;
    }
    if (this.widget.model.currentData.length === 0) {
      return;
    }

    var children = this.widget.el.find("tbody").children().length;
    for (var j = 0; j < children; j += 1) {
      for (var i = 0; i < this.widget.settings.config.style.length; i += 1) {
        ...
        var currSev = this.widget.model.currentData[currRow].sev,
          currState = this.widget.model.currentData[currRow].sta,
          rowEligible = this._rowEligibility(
            currState,
            currSev,
            this.widget.settings.config.style[i]
          );
        ...
        if (rowEligible) {
          this.widget.el
            ...
            .children("tr:eq(" + j + ")")
            .addClass(
              "widgets_brXtended_UserListStyle_style_style" +
                this.widget.settings.config.style[i].namePos
            );
        }
      }
    }
  ...
  p._colorMeBlueHelp = function (j) {
    if (this.dt.exists()) {
      var offset =
        this.dt.getFilteredPage() * this.widget.settings.itemsPerPage + j;
  ...
  p._fetchDataTableOptions = function () {
    var options = SuperClass.prototype._fetchDataTableOptions.apply(this);
    options.order =
      this.widget.settings.config.sort.length === 0
        ? [[this.widget.settings.childrenIdList.length, "asc"]]
        : this.widget.settings.config.sort;
    return options;
  };
