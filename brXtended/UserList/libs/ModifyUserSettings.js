define([
  "widgets/brXtended/UserList/libs/UserSettingsWidget",
  "brease/core/StringUtils",
  "brease/events/BreaseEvent",
], function (SuperClass, StringUtils, BreaseEvent) {
  "use strict";

  /**
   * @class widgets.brXtended.UserList.libs.ModifyUserSettings
  ...
  var UserClass = SuperClass.extend(function UserSetting(
      dialog,
      widget,
      lang,
      texts
  ...
  p.initialize = function () {
    ...
    this.config.data = this.widget.model.getRowFromCurrentData(
      this.widget.controller.getSelectedItem()
    );
  ...
  p._createWidgets = function () {
    ...
    row.push(
      this._getButton(
        ...
        this.texts.BTN_CHANGE_PASSWORD,
        LEFT_COL_1,
        TOP_ROW_2,
        WIDTH_COL_1
      )
    ...
    row.push(
      this._getPassword(
        "Password_Confirm",
        this.texts.PLH_CONFIRM_PASSWORD,
        LEFT_COL_1,
        TOP_ROW_3,
        WIDTH_COL_1,
        false
    ...
    row.push(
      this._getGroupBox(
        "GroupBox_Roles",
        this.texts.ROLES_HEADER,
        this.config.names.roles,
        LEFT_COL_2,
        TOP_ROW_0,
        WIDTH_GB_0,
        HEIGHT_GB_0,
    ...
    row.push(
      this._getGroupBox(
        "GroupBox_PasswordRules",
        this.texts.ERROR_HEADER,
        this.config.names.pwrules,
        LEFT_COL_0,
        TOP_ROW_4,
        WIDTH_GB_1,
        HEIGHT_GB_1,
    ...
    if (user.userID !== data.userName && user.isAuthenticated) {
      var mdi = "widgets/brXtended/UserList/assets/Accept.svg"; //MouseDownImage));
      row.push(
        this._getCheckBox(
          "Button_Reset_Lock",
          this.texts.BTN_RESET_LOCK,
          LEFT_COL_1,
          TOP_ROW_5,
          WIDTH_COL_1,
          true,
  ...
  p._handleChanges = function (e) {
    var info = {},
      passwordChange,
      results = {},
      omitPasswordValidation = this.dialog
        .getWidgetByName("Button_Change_Password")
        .isVisible();

    this._getDataFromForms(info);
    passwordChange =
      info.hasOwnProperty("newPassword") ||
      info.hasOwnProperty("confirmPassword");

    var updatePasswordVis =
      e.target.id.includes("Password_Password") ||
      e.target.id.includes("Password_Confirm");
    results.newPassword = omitPasswordValidation;
    results.confirmPassword = omitPasswordValidation;

    if (passwordChange) {
      results.newPassword = this._handlePassword(
        info.newPassword,
        updatePasswordVis
      );
      results.confirmPassword = this._handlePasswordConfirm(
        info,
        updatePasswordVis
      );
    }
    ...
    this._handleClosingButton(
      results.newPassword && results.confirmPassword && results.roles
    );
  };
  ...
  p._handlePassword = function (pwd, toggleVis) {
    ...
    if (toggleVis) {
      this._updateWidgetRulesHandler(
        this.config.names.pwrules + "_alpha",
        result.alphanumeric
      );
      this._updateWidgetRulesHandler(
        this.config.names.pwrules + "_mixed",
        result.mixedCase
      );
      this._updateWidgetRulesHandler(
        this.config.names.pwrules + "_min",
        result.minLength
      );
      this._updateWidgetRulesHandler(
        this.config.names.pwrules + "_special",
        result.specialChar
      );
    }
    return (
  ...
  p._handlePasswordConfirm = function (info, toggleVis) {
    ...
    if (toggleVis) {
      this._updateWidgetVisibility(
        "Label_" + this.config.names.pwrules + "_pwd_diff",
        validatedData.arInputs.includes("confirmPassword")
      );
      this._updateWidgetVisibility(
        "Image_" + this.config.names.pwrules + "_pwd_diff",
        validatedData.arInputs.includes("confirmPassword")
      );
    }
  ...
  p._updateWidgetRules = function (name, result, img) {
    ...
    if (cb) {
      ...
      if (img) {
        var src = result
          ? "widgets/brXtended/UserList/assets/Accept.svg"
          : "widgets/brXtended/UserList/assets/Reject.svg";
        cb.setImage(src);
      }
      cb.setVisible(true);
    }
  ...
  p._widgetCollectStateBeforeClosing = function (e) {
    if (this.validData) {
      ...
      var roles = this._getRolesFromForm();
      var rolesUpdated = !(
        roles.length === this.config.data.roles.length &&
        this.config.data.roles.every(function (value, index) {
          return value === roles[index];
        })
      );
      if (rolesUpdated) {
        data.roles = roles;
  ...
  p._getLockedUser = function () {
    var w = this.dialog.getWidgetByName("Button_Reset_Lock"),
      ret = false;
    if (w) {
      ret = w.getValue();
    }
  ...
  p._addContentRules = function (content, r) {
    var info = {
      mdi: "widgets/brXtended/UserList/assets/Accept.svg", //mouseDownImage
      mui: "widgets/brXtended/UserList/assets/Reject.svg", //mouseUpImage
    ...
    if (this.config.policies.alphanumeric) {
      ...
      flp.content = [
        ...
        this._getLabel(
          "Label_" + r + "_alpha",
          this.config.textKeys.alphanumeric,
          left,
          top,
          info.w,
          info.cbVisible,
    ...
    if (this.config.policies.mixedCase) {
      ...
      flp.content = [
        ...
        this._getLabel(
          "Label_" + r + "_mixed",
          this.config.textKeys.mixedCase,
          left,
          top,
          info.w,
          info.cbVisible,
    ...
    if (this.config.policies.minLength > 0) {
      ...
      this.config.textKeys.minLength = this.config.textKeys.minLength.replace(
        "{1}",
        this.config.policies.minLength
      );
      flp.content = [
        this._getImage(
          "Image_" + r + "_min",
        ...
        this._getLabel(
          "Label_" + r + "_min",
          this.config.textKeys.minLength,
          left,
          top,
          info.w,
          info.cbVisible,
    ...
    if (this.config.policies.specialChar) {
      ...
      flp.content = [
        ...
        this._getLabel(
          "Label_" + r + "_special",
          this.config.textKeys.specialChar,
          left,
          top,
          info.w,
          info.cbVisible,
    ...
    flp.content = [
      ...
      this._getLabel(
        "Label_" + r + "_pwd_diff",
        this.texts.PASSWORD_PASSWORDS_DIFFERENT,
        left,
        top,
        info.w,
        info.cbVisible,
