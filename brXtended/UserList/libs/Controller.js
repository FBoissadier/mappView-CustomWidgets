define([
  ...
  "widgets/brXtended/UserList/libs/Dialogue",
], function (SuperClass, BreaseEvent, ConfigDialogue) {
  "use strict";
  /**
   * @class widgets.brXtended.UserList.libs.Controller
  ...
  p._addClassSpecificEventListeners = function (listener) {
    ...
    document.body.addEventListener(
      BreaseEvent.USER_CHANGED,
      listener._bind("_loginChanged")
    );
  };
  ...
  p._contentActivatedHandler = function (e) {
    if (e.detail.contentId === this.widget.settings.parentContentId && this) {
      document.body.removeEventListener(
        BreaseEvent.CONTENT_ACTIVATED,
        this._bind("_contentActivatedHandler")
      );
  ...
  p.getEventItem = function (item) {
    ...
    var value = {
      ...
      LastLogin:
        data.lastLogin !== null ? data.lastLogin.substring(0, 23) + "Z" : "",
    };
    return value;
  };
