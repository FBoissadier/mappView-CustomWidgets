define([
  ...
  /**
   * @class widgets.brXtended.UserList.libs.AddUserSettings
  ...
  p._handleChanges = function (e) {
    ...
    result &= this._handlePasswordConfirm(
      info,
      updatePasswordConfirmVis || updatePasswordVis
    );
  ...
  p._handleUserName = function (info, toggleVis) {
    ...
    if (toggleVis) {
      this._updateWidgetRules(
        "Label_" + this.config.names.pwrules + "_username_min",
        validatedData.isValid
      );
      this._updateWidgetRules(
        "Image_" + this.config.names.pwrules + "_username_min",
        validatedData.isValid,
        true
      );
    }
  ...
  p._addContentRules = function (content, r, w) {
    ...
    if (this.config.userNameMinLength > 0) {
      this.config.textKeys.userNameMinLength =
        this.config.textKeys.userNameMinLength.replace(
          "{1}",
          this.config.userNameMinLength
        );
  ...
  p._addContentRoles = function (content, r, w) {
    var roles = this.config.roles,
      ...
      mdi = "widgets/brXtended/UserList/assets/Accept.svg";
