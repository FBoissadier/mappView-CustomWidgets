define(["widgets/brease/TableWidget/libs/Model", "brease/enum/Enum"], function (
  SuperClass,
  Enum
) {
  "use strict";
  /**
   * @class widgets.brXtended.UserList.libs.Model
  ...
  p._addSVGToRow = function (row) {
    ...
    if (row.isLocked) {
      locked =
        this.widget.settings.imageIsLocked === ""
          ? "widgets/brXtended/UserList/assets/IsLocked.svg"
          : this.widget.settings.imageIsLocked;
      row.isLocked =
        '<img src="' +
        locked +
        '" /><div class="stateIdent">' +
        row.isLocked +
        "</div>";
    } else {
    ...
    if (row.isAdmin) {
      admin =
        this.widget.settings.imageIsAdmin === ""
          ? "widgets/brXtended/UserList/assets/IsAdmin.svg"
          : this.widget.settings.imageIsAdmin;
      row.isAdmin =
        '<img src="' +
        admin +
      ...
      // admin = (this.widget.settings.imageIsNotAdmin === '') ? 'widgets/brXtended/UserList/assets/NotAdmin.svg' : this.widget.settings.IsNotAdmin;
  ...
  p._successLoadUserList = function (data) {
    ...
    var loggedUserData = this.searchLoggedUserInUserlist(
      this.getUser().userID,
      data
    );
    if (loggedUserData) {
  ...
  p._successRetrieveEditUserSameLevel = function (response) {
    ...
    editUserWithSameLevel = response
      ? response.EditUsersWithSameUserLevel
      : false;
  ...
  p.deleteUser = function (deferred) {
    var model = this,
      userData = this.widget.model.getRowFromCurrentData(
    ...
    return [
      {
        ...
        isAdmin: true,
        isLocked: false,
        lastLogin: "1970-01-01T00:00:01.000Z",
      },
      {
        userName: "Tiberius",
        fullName: "Tiberius Caesar Augustus",
        roles: ["Admin", "Imperator"],
        isAdmin: true,
        isLocked: false,
        lastLogin: "1970-01-01T11:33:15.141Z",
      },
      {
        userName: "Caligula",
        fullName: "Gaius Caesar Augustus Germanicus",
        roles: ["Service", "Imperator"],
        isAdmin: true,
        isLocked: true,
        lastLogin: "1970-01-01T11:31:56.541Z",
      },
      {
        userName: "Claudius",
        fullName: "Tiberius Claudius Caesar Augustus Germanicus",
        roles: ["Operator", "Imperator"],
        isAdmin: false,
        isLocked: true,
        lastLogin: "1970-01-01T11:33:15.141Z",
      },
      {
        ...
        isAdmin: false,
        isLocked: true,
        lastLogin: "1970-01-01T11:31:56.341Z",
      },
      {
        userName: "Vespasian",
        fullName: "Caesar Vespasianus Augustus",
        roles: ["Service", "Imperator"],
        isAdmin: false,
        isLocked: false,
        lastLogin: "1970-01-01T11:31:55.741Z",
      },
      {
        userName: "Aurelius",
        fullName: "Marcus Aurelius Antonius",
        roles: ["Operator", "Imperator"],
        isAdmin: false,
        isLocked: false,
        lastLogin: "1970-01-01T11:31:55.865Z",
      },
    ];
  }
